...
Bot.prototype = {
    ...
    chooseNextRandomCombination: function(){
        var keys = Object.keys(this.remainders.elements);
        self.tmp = keys[Math.floor(keys.length * Math.random())];

        return this.remainders.elements[self.tmp];
    },
...
SetReducerFactory.prototype = {
    getSetReducer: function(checkResult){
        if(checkResult.isOut){
            ...
            if(checkResult.ballCount == 1) return this.oneBallSetReducer;
            else if(checkResult.ballCount == 2) return this.twoBallSetReducer;
            else if(checkResult.ballCount == 3) return this.threeBallSetReducer;
        }else if(checkResult.ballCount == 0){
            if(checkResult.strikeCount == 1) return this.oneStrikeSetReducer;
            else if(checkResult.strikeCount == 2) return this.twoStrikeSetReducer;
        }else{
